...
export default class CrxService {
    ...
    statusPromise.
      then( ( status ) => {
          if ( status ) {
            ...
            let cmd = new Command();
            cmd.cmd = "up";
            cmd.arg( "." + CONTEXT.pathSeparator + "jcr_root" + CONTEXT.payloadPath );
  ...
  getFilterPath() {
    return "." + CONTEXT.pathSeparator + "META-INF" +
                 CONTEXT.pathSeparator + "vault" +
                 CONTEXT.pathSeparator + "filter.xml";
  }
