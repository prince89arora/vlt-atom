...
/**
 * status modal content handler. Validate status for given path in project and prepare
...
export default class StatusView {
    ...
    prepareHeader() {
        ...
        var basePath = document.createElement("p");
        let path_display = this.data.path.substring( this.data.path.indexOf('jcr_root') + 8, this.data.path.length );
        basePath.innerHTML = "Base: " + path_display;

        this.statusContainer.appendChild(title);
        this.statusContainer.appendChild(basePath);
